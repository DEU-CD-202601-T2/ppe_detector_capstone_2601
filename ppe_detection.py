...
# ══════════════════════════════════════════════════════════
# wrist ROI 박스 (팔꿈치 방향으로 박스 방향 결정)
# ══════════════════════════════════════════════════════════
...
yolo_model = YOLO("models/yolov8s-worldv2.pt")
yolo_model.model.float()
yolo_model.to("cuda")
# 초기화 때 한 번만 set_classes 호출
yolo_model.set_classes(WORLD_CLASSES)

pose_model = YOLO("models/yolo11n-pose.pt")
pose_model.model.float()
pose_model.to("cuda")
